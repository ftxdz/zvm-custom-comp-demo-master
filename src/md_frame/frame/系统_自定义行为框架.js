...
// main函数内可以使用的对象及工具函数有：context、query、mutation、batch_query、batch_mutation、runGqlImp、errlog、setReturn
...
function callActionflow(args = {}, actionflow_id = undefined) {
	if (!actionflow_id) {
		actionflow_id = system?.actionflow_id || system?.actionflow_config?.actionflow_id;
...
function setReturn(res, res_status = "成功", res_msg = undefined, is_last_res = false) {
	...
	if (typeof result != "object") {
		...
		result = {
			status: "失败",
			msg: result?.message || result || "系统内部错误"
		}
	}
...
function getAuthtoken(args = {}) {
	...
	if (now_time >= authtoken?.exp_time) {
		...
		let set_exp_seconds = authtoken?.set_exp_seconds || 300;
		// 更新数据表
